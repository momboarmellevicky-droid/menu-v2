...
Minimum 2 fichiers, maximum 6 fichiers. Le fichier racine doit obligatoirement s'appeler "/src/App.tsx" et exporter un composant par défaut. Rien avant le premier ###FILE:, rien après le dernier ###ENDFILE###. N'entoure JAMAIS le contenu d'un fichier de balises Markdown \`\`\` — ni au début, ni à la fin d'un bloc. Le texte entre ###FILE:chemin### et ###ENDFILE### doit être EXCLUSIVEMENT le code source, rien d'autre.`,
  },
  {
    ...
    name: 'Testeur',
    role: 'tester',
    systemPrompt: `Tu es un QA engineer senior spécialisé en simulation d'exécution mentale (dry-run).
Pour CHAQUE fonction avec état (useState, useEffect, setInterval, setTimeout), trace mentalement l'exécution étape par étape comme si tu étais le moteur JavaScript :
1. Identifie chaque variable d'état et son cycle de vie exact.
2. Simule le déroulement temporel réel (que se passe-t-il pendant un délai/timer/animation, pas seulement avant et après).
3. Cherche spécifiquement les race conditions : un état qui change AVANT qu'une garde ne soit active.
4. Vérifie que chaque condition de blocage (isPaused, isReady, status==='playing') est RÉELLEMENT vérifiée à CHAQUE point où une action pourrait se produire.
Retourne en JSON :
- errors: tableau d'erreurs critiques trouvées, CHAQUE erreur doit citer la ligne/fonction précise et le scénario exact de trace qui la déclenche
...
`)
    ...
    let currentCode = dev.output
    let test = await callAgent(AGENTS[4], currentCode)
    results.push(test)

    let finalCode = currentCode
    for (let round = 0; round < 2; round++) {
      onProgress?.('Optimiseur', 85 + round * 5)
...
Optimise le code en appliquant les corrections suggérées. Corrige en priorité toute race condition ou garde d'état non vérifiée signalée par le rapport.
      `)
      if (optimized.status === 'error') break
      finalCode = optimized.output
      currentCode = finalCode

      const hasCriticalErrors = /"errors"\s*:\s*\[\s*\{/.test(test.output)
      if (!hasCriticalErrors) break

      onProgress?.('Testeur', 90 + round * 5)
      test = await callAgent(AGENTS[4], currentCode)
      results.push(test)
    }
    results.push({ agent: 'Optimiseur', output: finalCode, status: 'success', timestamp: new Date().toISOString() })
...
FICHIERS MANQUANTS À CRÉER (référencés par un impo
